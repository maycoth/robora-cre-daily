...
export default function HowItWorksSteps() {
  return (
    <section id="how-it-works" className="relative w-full bg-[#F5F7FA] py-20 md:py-28 overflow-hidden">
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="relative">
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-12 left-[60%] w-full h-0.5 bg-[#E5E7EB]">
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 border-t-2 border-r-2 border-[#E5E7EB] transform rotate-45"></div>
                </div>
              )}
              
              <div className="relative z-10 bg-white rounded-2xl p-6 h-full shadow-sm border border-[#E5E7EB]">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-12 h-12 bg-[#003D7A] rounded-xl flex items-center justify-center">
                    <step.icon className="w-6 h-6 text-[#FFC700]" />
                  </div>
                  <span className="text-5xl font-bold text-[#E5E7EB]">{step.id}</span>
                </div>
                <h3 className="text-lg font-bold text-[#003D7A] mb-2">{step.title}</h3>
